...
public class PaginatesServiceImpl {
	public PaginatesDto GetinfoPaginates(int totalData, int limit, int currentPage) {
		...
		paginate.setTotalPage(SetinfotTotalPage(totalData, limit));
		paginate.setCurrentPage(CheckCurrentPage(currentPage, paginate.getTotalPage()) );
		
		int start= FindStatr( paginate.getCurrentPage(), limit);
	...
	private int FindStatr(int currentPage, int limit) {
	...
	private int SetinfotTotalPage(int totalData, int limit) {
		int totalPage = 0;
